...
### The Objective:
Build an end-to-end search query and advertising gap analysis engine:
1. Export massive datasets of search queries, metrics, and attributes from Google Search Console, Google Ads, and spreadsheets.
2. Clean noise, normalize typos, and deduplicate entries.
3. Compare the dataset against existing active campaigns and competitor intelligence.
4. Calculate the "delta": identify high-intent search terms we are missing and pinpoint exact keyword gaps.
5. Generate polished, ready-to-run ad copy for those new clusters and export a clean file directly importable into the ad platform.
