...
graph TD
    ...
    subgraph "Серверная Инфраструктура"
      ...
      C -->|Динамические API Роуты| E["Astro Node.js SSR"]
...
1. **Защита от XSS**: Все пользовательские вводы из форм очищаются через `sanitize-html` на бэкенде Astro (Node.js) для исключения инъекций скриптов.
...
3. **Изоляция Контейнеров**: Docker-сборка базируется на Node.js 24 Alpine с жестко заданным запуском процессов от имени пользователя без root-прав (non-root execution).
